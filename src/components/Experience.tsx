...
const Experience: React.FC = () => {
  ...
  const getIconForRole = (title: string) => {
    ...
    if (title.includes('Head') || title.includes('Coordinator')) return Target;
    return Building;
  };
      ...
      <div className="container-custom relative z-10">
              ...
              >
                ...
                <div className={`w-full md:w-1/2 pl-20 md:pl-0 ${
                  ...
                  <div className={`card card-hover p-8 ${
                    isEven 
                      ? 'border-l-4 border-coral-400' 
                      : 'border-r-4 border-burnt-400'
                  }`}>
                    {/* Period Badge */}
                    <div className={`inline-flex items-center px-4 py-2 rounded-full text-sm font-medium mb-4 ${
                      isEven 
                        ? 'bg-coral-100 text-coral-800' 
                        : 'bg-burnt-100 text-burnt-800'
                    }`}>
                    ...
                    <h3 className="font-serif font-semibold text-2xl text-ink-900 mb-2">
                      {exp.title}
                    </h3>
                      ...
                      {exp.description.map((item, idx) => (
                        <div key={idx} className="flex items-start">
                          <div className={`w-2 h-2 rounded-full mt-2 mr-3 flex-shrink-0 ${
                            isEven ? 'bg-coral-400' : 'bg-burnt-400'
                          }`}></div>
                    ...
                    {exp.tags && (
                      <div className="flex flex-wrap gap-2">
                        {exp.tags.map((tag, idx) => (
                          <span
                            key={idx}
                            className="px-3 py-1 text-xs font-medium rounded-full bg-beige-200 text-ink-600 border border-beige-300"
                          >
                            {tag}
                          </span>
                        ))}
        ...
        >
          <motion.div variants={cardVariants} className="text-center">
            ...
            <div className="font-serif font-bold text-3xl text-ink-900 mb-1">7+</div>
          ...
          <motion.div variants={cardVariants} className="text-center">
            ...
            <div className="font-serif font-bold text-3xl text-ink-900 mb-1">6+</div>
          ...
          <motion.div variants={cardVariants} className="text-center">
            ...
            <div className="font-serif font-bold text-3xl text-ink-900 mb-1">60+</div>
            <div className="text-ink-600 font-medium">Developers Mentored</div>
          </motion.div>
        </motion.div>
      </div>
    </section>
